...
from geotools.coordinates import Ellipsoid
...
def compute_surface(lb_pixel, rb_pixel, ub_pixel, bb_pixel, geo_type: str, ellipsoid: Ellipsoid):
    ...
    def ellipsoid_normal():
        return ellipsoid.semi_major_axis/sqrt(1 - (ellipsoid.eccentricity ** 2) * (sin(latitude * pi/180)) ** 2)

    def local_sphere_radius():

        p = ellipsoid.semi_major_axis * (1 - ellipsoid.eccentricity ** 2)/(1 - ellipsoid.eccentricity ** 2 *
...
if __name__ == "__main__":
    ...
    centroid_2 = (geo2.longitude_mesh, geo2.latitude_mesh)

    for (longitude, latitude) in zip(np.nditer(geo.longitude_mesh), np.nditer(geo.latitude_mesh)):
