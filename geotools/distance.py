...
def great_circle(latitude_1, latitude_2, delta_longitude, ellipsoid: Ellipsoid):
    ...
    def ellipsoid_normal():
        return ellipsoid.semi_major_axis/sqrt(1 - (ellipsoid.eccentricity ** 2) * (sin(latitude_1 * pi/180))
                                              ** 2)

    def local_sphere_radius():

        p = ellipsoid.semi_major_axis * (1 - ellipsoid.eccentricity ** 2)/(1 - ellipsoid.eccentricity ** 2 * sin(
...
if __name__ == '__main__':
    from geotools.coordinates import GeoGrid, Ellipsoid
