...
from gis_tools.coordinates import GeoGrid
from gis_tools.conversion import raster_to_array, array_to_raster
from gis_tools.exceptions import RasterMapError, GeoGridError
from gis_tools.files import RasterTempFile, ShapeTempFile
from gis_tools.layer import PolygonLayer
from gis_tools.projections import proj4_from_raster, is_equal, proj4_from, wkt_from, srs_from, ellipsoid_from
from gis_tools.surface import compute_surface
...
class RasterMap:
    ...
    _ogr_drv = ogr.GetDriverByName('ESRI Shapefile')
    ...
    def plot(self, ax=None, cmap=None, colorbar=False, colorbar_title=None, **kwargs):
        ...
        img = ax.imshow(self.raster_array, extent=extent, cmap=cmap, **kwargs)
    ...
    def polygonize(self, field_name, is_8_connected=False):
    ...
    def _gdal_polygonize(self, out_shp, layer_name, field_name, is_8_connected):
        ...
        with GdalOpen(self.raster_file) as src_ds:
            ...
            dst_ds = self._ogr_drv.CreateDataSource(out_shp)
...
if __name__ == '__main__':
    ...
    classification = RasterMap(np.zeros((slope.y_size, slope.x_size)), slope.geo_grid, crs=slope.crs,
                               no_data_value=0)
    classification[slope <= 20] = 1
    # classification.plot()
    classification_sieved = classification.apply_filter("sieve", 10)
    test_polygon = classification_sieved.polygonize("slope")
    test_polygon = test_polygon.simplify(100)
    # classification_sieved.plot()
    test_polygon.to_file("/home/benjamin/Documents/Post-doc Guyane/Data/Base layer/flat_slope_layer_biomass.shp")
    # test_polygon.plot()
    # plt.show()
