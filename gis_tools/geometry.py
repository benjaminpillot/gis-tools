...
import nxmetis
from gis_tools.coordinates import r_tree_idx
...
def partition_polygon(polygon, unit_area, weight_attr="area", disaggregation_factor=8, recursive=False):
    ...
    # graph.graph['node_weight_attr'] = weight_attr
    ...
    # _, partition = metis.part_graph(graph, nparts)
    # _, partition = pymetis.part_graph(nparts, graph)

    options = nxmetis.MetisOptions(contig=True, ncuts=10, iptype=3, rtype=2)
...
if __name__ == "__main__":
    ...
    poly_collection = partition_polygon(polygon_, 10000000, weight_attr="area",
