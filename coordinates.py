...
from geotools.projections import proj4_from_raster
from utils.check.type import is_iterable
from utils.sys.reader import read_hdr
from utils.check import *
from geotools.exceptions import GeoGridError, GeoGridWarning

# __all__ = ["Ellipsoid", "GeoGrid", "Location", "LocationPoint", "TIME_ZONE"]
# __version__ = '0.1'
...
class Ellipsoid:
    ...
    def __init__(self, ellipsoid_model: str):
        ...
        self._flattening = 1 / REF_ELLIPSOID[ellipsoid_model]['rf']
        self._semi_major_axis = REF_ELLIPSOID[ellipsoid_model]['a']
        self._semi_minor_axis = self._semi_major_axis - self._semi_major_axis * self._flattening
        self._eccentricity = math.sqrt(1 - (self._semi_minor_axis**2/self._semi_major_axis**2))
...
class Location(metaclass=_SpecialMeta):
...
class GeoGrid(Geogrid):
    ...
    """
    latitude_mesh = protected_property('latitude')
    longitude_mesh = protected_property('longitude')
    ...
    def __getitem__(self, key):
        ...
        if key.__class__ == tuple:
            if key[0].__class__ == slice or key[1].__class__ == slice:
                try:
                    lat_south, lat_north = self.lats[key[0]][0], self.lats[key[0]][-1]
                except TypeError:
                    lat_south = lat_north = self.lats[key[0]]
                try:
                    lon_west, lon_east = self.lons[key[1]][0], self.lons[key[1]][-1]
                except TypeError:
                    lon_west = lon_east = self.lons[key[1]]
                ...
                return self.latitude_mesh.__getitem__(key), self.longitude_mesh.__getitem__(key)
            else:
    ...
    def from_geo_file(geo_file: str, res, buffer_accuracy=1, to_crs: str=''):
        ...
        from geotools.projections import proj4_from
